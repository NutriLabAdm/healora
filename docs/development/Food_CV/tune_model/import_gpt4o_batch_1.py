...
with open(f'{TUNE_DIR}/gpt4o.json', 'r', encoding='utf-8') as f:
    gpt4o = json.load(f)

with open(f'{TUNE_DIR}/gpt4o_batch_01_03.json', 'r', encoding='utf-8') as f:
    batch = json.load(f)
...
for entry in batch:
    ...
    gpt4o['dishes'][did] = dish_obj

gpt4o['date_updated'] = '2026-06-14'
gpt4o['notes'] = 'GPT — анализ 3 блюд (01-03)'

out = f'{TUNE_DIR}/gpt4o.json'
with open(out, 'w', encoding='utf-8') as f:
    json.dump(gpt4o, f, ensure_ascii=False, indent=2)

done = sum(1 for d in gpt4o['dishes'].values() if d['response']['calories_predicted'] is not None)
print(f'gpt4o.json: {done}/23 done')
print()
print(f'{"#":>3} {"Dish":30s} {"GS_Cal":>6} {"Pred":>5} {"MAE%":>6} {"Match":>5} {"Conf":>4}')
print('-' * 65)
for did in [str(i).zfill(2) for i in range(1, 4)]:
    d = gpt4o['dishes'].get(did, {})
    r = d.get('response', {})
